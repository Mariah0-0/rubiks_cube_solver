...
def inputcase(case):
    mycon = ms.connect(host='localhost', username='root', passwd='root', database='12cs_2122')
    ...
    while True:
        colours = []
        mycursor.execute('delete from cube1')
        for i in range(24):
            while True:
                ...
                if ver1(colour,colours,positions,i):
                    ...
                    break
                print('\nInvalid value\n')
            query = 'insert into cube1 values(\''+positions[i]+'\', \''+colour+'\')'
            mycursor.execute(query)
            mycon.commit()
        mycursor.execute('select * from cube1')
